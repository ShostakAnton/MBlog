...
class UserTweet(APIView):
    """Твиты пользователя"""
    permission_classes = [permissions.IsAuthenticated]  # тлько авторизированые пользователи

    def get(self, request):  # получение твитов пользователя
        tweets = Post.objects.filter(
            user=request.user)  # запрос в бд, с фильтром, где user равен user который послал запрос
        ser = PostSerializer(tweets, many=True)  # выполнмем сериализацию
        return Response(ser.data)

    def post(self, request):  # запрос на добавления сообщения
